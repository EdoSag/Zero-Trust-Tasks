...
-- Keep a single row per user if duplicates were inserted before UNIQUE(user_id).
delete from public.encrypted_tasks a
using public.encrypted_tasks b
where a.user_id = b.user_id
  and a.id < b.id;

alter table public.encrypted_tasks
drop constraint if exists encrypted_tasks_user_id_key;

alter table public.encrypted_tasks
add constraint encrypted_tasks_user_id_key unique (user_id);
